/**
 * One-off: place a $1 limit order on the current 5m BTC market, winning side (B4 wallet).
 * Uses .env (POLYMARKET_*, derive, HTTPS_PROXY). Run on D2:
 *   cd /root/cursorbot && node dist/scripts/place-b4-test-order.js
 */
...
async function main() {
  ...
  console.log('[B4 test] 5m slug:', slug);

  await withPolyProxy(async () => {
    const market = await getPolyMarketBySlug(slug);
    if (!market) {
      console.error('[B4 test] Market not found');
      process.exit(1);
    }
    ...
    if (!tokenId) {
      console.error('[B4 test] No token for', side);
      process.exit(1);
    }
    const limitPrice = 0.97;
    const shares = Math.max(2, Math.ceil(TEST_AMOUNT_USD / limitPrice));
    console.log('[B4 test] Winning side:', side, '| yes=', yesPrice.toFixed(3), 'no=', noPrice.toFixed(3), '| limit', limitPrice, 'shares', shares);
    ...
    console.log('[B4 test] Placing $' + TEST_AMOUNT_USD + ' GTC limit BUY', side, '...');
    const result = await client.createAndPostOrder(
      { tokenID: tokenId, price: limitPrice, size: shares, side: Side.BUY },
      { tickSize, negRisk },
      OrderType.GTC
    );
    const orderId = (result as { orderID?: string; orderId?: string })?.orderID ?? (result as { orderId?: string })?.orderId;
    if (orderId) {
      console.log('[B4 test] Order placed:', orderId);
    } else {
      console.log('[B4 test] Raw result:', JSON.stringify(result));
    }
  });
}
