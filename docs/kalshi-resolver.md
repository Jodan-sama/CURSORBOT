...
## Cron (every 15 min at :03 and :18)

Run at 3 minutes after the 15‑minute window so it’s in a calm period (:03, :18, :33, :48).
...
```cron
3,18 * * * * cd /root/cursorbot && /usr/bin/npx tsx src/scripts/resolve-kalshi-outcomes.ts >> /var/log/cursorbot-kalshi-resolver.log 2>&1
```
