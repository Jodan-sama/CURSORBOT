...
**Wallets and “wrong bot” checks:** Each Polymarket bot must use a **different** wallet: B4 = D2 `.env` `POLYMARKET_FUNDER`; B123c = D2 `.env.b123c` `POLYMARKET_FUNDER`; B1/B2/B3 Poly = D1 `.env` `POLYMARKET_FUNDER`; B5 = D3 (`.env.b5` on D2 for resolver). If B123c and D1 share the same funder, a trade on that wallet could be from either; the **dashboard bot** (B1c vs B1) is whatever process called `logPosition` when the order was placed. To verify wallets and that only one process per service is running, on D2 run: `cd /root/cursorbot && npx tsx src/scripts/verify-wallets-and-processes.ts`. Then run `systemctl list-units "cursorbot*"` and `pgrep -af "b123c-run|b4-5m"` — you should see exactly one `cursorbot-b123c` and one `cursorbot-b4-5m`. **15m window time:** Logs use server time (D2 is often UTC+1). 9:30–9:45 AM ET = 14:30–14:45 UTC = 15:30–15:45 server (if UTC+1). To find a SOL place in that window: `ssh root@161.35.149.219 "journalctl -u cursorbot-b123c --since '2026-02-27 15:30:00' --until '2026-02-27 15:46:00' --no-pager | grep -E 'SOL|placed'"`. If there is no SOL place in that range but the dashboard shows a B1c/B2c/B3c Solana position for that market, either (1) two B123c processes were running (check with `pgrep -af b123c-run` on D2 — there must be exactly one), or (2) the position’s wallet is shared with D1 (B1/B2/B3); verify with the script above.
